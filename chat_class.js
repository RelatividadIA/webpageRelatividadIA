...
class Chat {
    ...
    renderConversationHistory() {
        let conversationElement = document.getElementById(this.#apiResponse);
    ...
    resetConversation() {
        document.getElementById(this.#chatID).value = '';
        document.getElementById(this.#apiResponse).innerHTML = ''; // Limpiar la visualización de la conversación
        this.#conversationHistory = [{ role: "system", content: this.#trainingPrompt }]; // Reiniciar con el prompt de entrenamiento
        this.renderConversationHistory();
    }
    
    sendMessage() {
        let userPrompt = document.getElementById(this.#chatID).value.trim();
    ...
    subscribeEvents() {
        // Agregar evento al formulario para manejar el envío de mensajes
        document.getElementById(String(this.#promptForm)).addEventListener("submit", function(event){
        event.preventDefault();
        this.sendMessage();
        });

        // Evento para detectar la tecla Enter y enviar el mensaje
        document.getElementById(String(this.#chatID)).addEventListener('keypress', function(event) {
            if (event.key === 'Enter') {
        ...
        document.querySelector(String(this.#btnClear)).addEventListener('click', this.resetConversation);
    }
}
